...
# initialization
#KKckpt_dir = './output/%s/checkpoints' % experiment_name
ckpt_dir = './output/tempcheck' #KK
...
try:
    ...
    for idx, batch in enumerate(te_data):
        ...
        oriImg = sample[:,:,0:128,:]  #KK
        recImg = sample[:,:,140:268,:]  #KK
        baldImg = sample[:,:,269:397,:]  #KK
        bangImg = sample[:,:,398:526,:]  #KK
        blackImg = sample[:,:,527:655,:]  #KK
        blondImg = sample[:,:,656:784,:]  #KK
        brownImg = sample[:,:,785:913,:]  #KK
        eyebrowImg = sample[:,:,914:1042,:]  #KK
        glassImg = sample[:,:,1043:1171,:]  #KK
        genderImg = sample[:,:,1172:1300,:]  #KK
        mouthImg = sample[:,:,1301:1429,:]  #KK
        bearImg = sample[:,:,1430:1558,:]  #KK
        nobearImg = sample[:,:,1559:1687,:]  #KK
        paleImg = sample[:,:,1688:1816,:]  #KK
        oldImg = sample[:,:,1817:1945,:]  #KK       
        ...
        im.imwrite(oriImg.squeeze(0), '%s/ori_%06d%s.png' % (save_dir,
                                                        idx + 182638 if img is None else img[idx], 
                                                        '_%s'%(str(test_atts)) if multi_atts else ''))

        im.imwrite(recImg.squeeze(0), '%s/rec_%06d%s.png' % (save_dir,
                                                        idx + 182638 if img is None else img[idx], 
                                                        '_%s'%(str(test_atts)) if multi_atts else ''))

        im.imwrite(baldImg.squeeze(0), '%s/bald_%06d%s.png' % (save_dir,
                                                        idx + 182638 if img is None else img[idx], 
                                                        '_%s'%(str(test_atts)) if multi_atts else ''))

        im.imwrite(bangImg.squeeze(0), '%s/bang_%06d%s.png' % (save_dir,
                                                        idx + 182638 if img is None else img[idx], 
                                                        '_%s'%(str(test_atts)) if multi_atts else ''))

        im.imwrite(blackImg.squeeze(0), '%s/black_%06d%s.png' % (save_dir,
                                                        idx + 182638 if img is None else img[idx], 
                                                        '_%s'%(str(test_atts)) if multi_atts else ''))

        im.imwrite(blondImg.squeeze(0), '%s/blond_%06d%s.png' % (save_dir,
                                                        idx + 182638 if img is None else img[idx], 
                                                        '_%s'%(str(test_atts)) if multi_atts else ''))

        im.imwrite(brownImg.squeeze(0), '%s/brown_%06d%s.png' % (save_dir,
                                                        idx + 182638 if img is None else img[idx], 
                                                        '_%s'%(str(test_atts)) if multi_atts else ''))

        im.imwrite(eyebrowImg.squeeze(0), '%s/eyebrow_%06d%s.png' % (save_dir,
                                                        idx + 182638 if img is None else img[idx], 
                                                        '_%s'%(str(test_atts)) if multi_atts else ''))

        im.imwrite(glassImg.squeeze(0), '%s/glass_%06d%s.png' % (save_dir,
                                                        idx + 182638 if img is None else img[idx], 
                                                        '_%s'%(str(test_atts)) if multi_atts else ''))

        im.imwrite(genderImg.squeeze(0), '%s/gender_%06d%s.png' % (save_dir,
                                                        idx + 182638 if img is None else img[idx], 
                                                        '_%s'%(str(test_atts)) if multi_atts else ''))

        im.imwrite(mouthImg.squeeze(0), '%s/mouth_%06d%s.png' % (save_dir,
                                                        idx + 182638 if img is None else img[idx], 
                                                        '_%s'%(str(test_atts)) if multi_atts else ''))

        im.imwrite(bearImg.squeeze(0), '%s/bear_%06d%s.png' % (save_dir,
                                                        idx + 182638 if img is None else img[idx], 
                                                        '_%s'%(str(test_atts)) if multi_atts else ''))

        im.imwrite(nobearImg.squeeze(0), '%s/nobear_%06d%s.png' % (save_dir,
                                                        idx + 182638 if img is None else img[idx], 
                                                        '_%s'%(str(test_atts)) if multi_atts else ''))

        im.imwrite(paleImg.squeeze(0), '%s/pale_%06d%s.png' % (save_dir,
                                                        idx + 182638 if img is None else img[idx], 
                                                        '_%s'%(str(test_atts)) if multi_atts else ''))

        im.imwrite(oldImg.squeeze(0), '%s/old_%06d%s.png' % (save_dir,
                                                        idx + 182638 if img is None else img[idx], 
                                                        '_%s'%(str(test_atts)) if multi_atts else ''))
